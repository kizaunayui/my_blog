...
- **全站版心**：桌面端使用最大 `1200px` 的 `.site-shell`，内容区保留至少 `24px` 内边距；禁止在 1280px 屏幕上把正文贴近视口边缘。
- **首页主信息**：Hero 文案最大宽度为 `760px`。中文引题与英文站名必须分行，并使用约 `0.56 : 1` 的字号比例建立主次。
- **首页动作区**：只保留一个实体主按钮；其他入口使用两列索引文字，不应再次做成五个同权胶囊按钮。
- **特色文章**：相对主文案向右缩进，最大宽度约 `52rem`，通过错位形成第二层网格，不与 Hero 标题等宽。
- **首页归档**：桌面端正文与侧栏约为 `2.15 : 0.85`，栏间距保持 `4rem–6.5rem`；文章日期列控制在 `8rem` 左右。
- **文章归档页**：标签索引栏固定约 `210px`，与文章区保留约 `5rem` 的桌面栏距。文章条目使用“日期 / 正文”双栏结构。
- **标签页**：桌面端固定两列而非三列，列间距保持 `3rem–6rem`；移动端恢复单列。
- **纵向节奏**：页面刊头与正文之间保留 `4rem–6.5rem`，普通文章条目上下留白约 `2.75rem–3rem`。装饰线不能代替真正的留白。
