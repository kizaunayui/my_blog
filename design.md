...
### 7.7 首页重设计（2026-09-08）

用户本轮明确授权重新设计首页，要求保留个人小标题、音乐、地点和天气等巧思。本节更新 7.2 中旧的首页不可重排限制；其他文章排版与无毛玻璃约束继续适用。

- 首页使用 `space-cover` 左侧站点介绍、右侧精选文章的开放双栏布局，移动端纵向排列。
- 三个内容栏目使用 `space-portals` 编号目录，并保留数据源中的栏目名称与说明。
- 最近更新使用 `space-journal`：左侧编号、横排日期／阅读时间／地点天气、下方标题摘要。
- 首页专用样式集中在 `css/editorial-polish.css`，不得覆盖文章内部的小标题或特殊正文样式。
- 保留 `Journal Archive`、`About Author`、`Trending Tags`、`Featured` 等原小标题。
- 随机／专属背景、音乐、头像光环、鼠标微交互、秒级阅读时间、地点天气、Anime 与 Reflexion 特殊排版均保留。

### 7.8 首访导航与编号修正

- 首页主按钮“浏览文章”直接定位最近更新；主题入口移至文章列表之后，形成明确阅读顺序。
- 精选文章显示“阅读这篇”文字提示，隐藏多余大号装饰编号。
- 文章列表编号使用正常文档流中的独立网格列，禁止绝对定位贴边或悬停向左移动，避免裁切。

### 7.9 精简首页（当前采用）

用户要求取消独立最新／精选文章，首页改为简短站点介绍直接衔接最近更新。取消独立精选、重复主题入口和每篇重复的阅读按钮；文章标题保留链接，顶部导航保留内容中心。弱化编号与标签，保留独立编号列避免裁切。作者与话题小标题、音乐、地点天气、专属背景和文章内部排版继续保留。本节取代 7.7、7.8 中的首页精选与主题入口布局。

### 7.10 杂志式视觉精修（2026-09-20）

- 保留艺术背景、透明布局、音乐与文章特殊排版，按背景明暗分档增加遮罩。
- 全站标题、日期、标签、按钮及页面顶部间距的统一规则集中在 `css/editorial-polish.css`。
- 青灰色为主要交互色，标签与日期降低字重和字距；列表增加纵向留白，标题保持衬线字体。
- 手机页面标题控制在约 2rem，文章标题约 1.6rem，并保持自然换行。
- 此次调整仅改变展示样式，不改变文章内容、导航结构或已删除项目总览的状态。

### 7.11 克制的阅读界面（2026-09-22）

- 保留艺术背景、透明布局、音乐、地点天气和特殊文章排版。全站外框由 `.site-shell` 统一，导航、首页与页脚共用 1080px 的对齐线，手机保留约 20px 的侧边距。
- 界面以暖白、灰绿与低饱和灰色区分标题、交互和辅助信息；遮罩继续按图片明暗分档，技术文章保留更深的独立遮罩。
- 首页维持简短介绍接文章列表；文章顺序为日期与阅读信息、标题、摘要、标签，减少阅读中断。日期使用 `YYYY.MM.DD`，地点天气和秒级阅读时间完整保留。
- 列表与标签不漂浮、不扫光；链接以文字颜色和细下划线反馈。作者入口仅用轻微箭头位移提示跳转，头像光环保持原有的悬停反馈。
- 音乐待机按钮保持静止，播放时使用低饱和细环呼吸；移动端触控尺寸为 44px。所有动效继续遵循 Reduced Motion。
- 首页规则在 `css/editorial-polish.css` 内按组件集中维护，后续调整直接修改对应规则，避免不断追加相互覆盖的版本块。
