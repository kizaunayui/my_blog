  ...
  return (
    <>
      ...
      <article className="py-10 sm:py-12">
        <header className="animate-fade-up mb-8 rounded-3xl border border-white/10 dark:border-white/5 bg-white/40 dark:bg-slate-950/20 px-6 py-12 text-center shadow-[0_8px_30px_rgba(0,0,0,0.02)] backdrop-blur-md sm:px-10">
          <dl>
        ...
        <section className="mb-8 rounded-3xl border border-white/10 dark:border-white/5 bg-white/40 dark:bg-slate-950/20 p-5 shadow-[0_8px_30px_rgba(0,0,0,0.02)] backdrop-blur-md sm:p-6">
          <div className="grid gap-5 md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] xl:grid-cols-[minmax(0,1.3fr)_minmax(0,0.8fr)_minmax(0,1fr)] xl:items-center">
            <div className="flex flex-wrap items-center gap-4">
              {authorDetails.map((author) => (
                <div className="flex items-center gap-3" key={author.name}>
                  ...
                  <div className="text-xs font-bold whitespace-nowrap">
                    ...
                    {author.twitter && (
                      <Link href={author.twitter} className="text-cyan-600 hover:text-cyan-700 dark:text-cyan-300 dark:hover:text-cyan-100 font-heading text-[10px] uppercase tracking-wider">
                        {author.twitter
              ...
              {record && (record.location || record.weather) && (
                <div className="flex flex-wrap items-center gap-2 text-xs font-medium text-gray-800 dark:text-slate-200">
                  <span className="rounded-full border border-white/20 bg-white/10 dark:border-white/10 dark:bg-slate-900/20 px-2.5 py-1 font-heading text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
                    记于
                  </span>
                  {record.location && <span>{record.location}</span>}
                  {record.weather && <span className="text-slate-500 dark:text-slate-400">{record.weather}</span>}
                </div>
              )}
            </div>
            ...
            {tags && tags.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-heading text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-slate-500">Tags</span>
                {tags.map((tag) => (
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs font-bold xl:justify-end font-heading uppercase tracking-wider">
              {prev && (prev.slug || prev.path) && (
                <Link href={articleHref(prev)} className="text-cyan-600 hover:text-cyan-700 dark:text-cyan-300 dark:hover:text-cyan-100">
                  ← Previous
                </Link>
              )}
              {next && (next.slug || next.path) && (
                <Link href={articleHref(next)} className="text-cyan-600 hover:text-cyan-700 dark:text-cyan-300 dark:hover:text-cyan-100">
                  Next →
                </Link>
              )}
              <Link
                href="/articles"
                className="rounded-full border border-white/20 bg-white/10 dark:bg-slate-900/30 px-4 py-2 text-slate-800 dark:text-white transition hover:bg-white/20"
